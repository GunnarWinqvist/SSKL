 ...
 * Edit user (edit_usr)
 *
 * Sidan presenterar ett formulär i QuickForm2 för alla uppgifter om en 
 * användare utom kontouppgifter. När alla uppgifter är korrekta så uppdateras 
 * databasen. Från sidan skickas man till PShowUser.
 * Input: 'id'
 */
...
if (isset($arrayResult[4])) { 
    ...
    while($row = $arrayResult[4]->fetch_object()) {
        $aMalsmanElev[$i] = array( 
            'id'=>$row->idPerson, 
            'fornamn'=>$row->fornamnPerson, 
            'efternamn'=>$row->efternamnPerson 
        );
        $i++;
    }
    $arrayResult[4]->close();
...
$formAction = WS_SITELINK . "?p=edit_usr&id=".$idPerson; 
$form = new HTML_QuickForm2('user', 'post', 
...
$fornamnPerson = $fsPerson->addElement('text', 'fornamn', 
    array('style' => 'width: 300px;'), 
    array('label' => 'Förnamn') );
$fornamnPerson->addRule('required', 'Fyll i förnamn');
$fornamnPerson->addRule('maxlength', 
    'Förnamnet är för långt för databasen.', 50);

$efternamnPerson = $fsPerson->addElement('text', 'efternamn', 
    array('style' => 'width: 300px;'), 
    array('label' => 'Efternamn') );
$efternamnPerson->addRule('required', 'Fyll i efternamn');
$efternamnPerson->addRule('maxlength', 
    'Efternamnet är för långt för databasen.', 50);

$ePostPerson = $fsPerson->addElement('text', 'ePost', 
    array('style' => 'width: 300px;'), 
...
$ePostPerson->addRule('maxlength', 
    'E-postadressen är för lång för databasen.', 50);

$mobilPerson = $fsPerson->addElement('text', 'mobil', 
    array('style' => 'width: 300px;'), 
    array('label' => 'Mobilnummer') );
$mobilPerson->addRule('maxlength', 
    'Mobilnumret är för långt för databasen.', 20);
...
if ($_SESSION['authorityUser'] == 'fnk' or  
    ...
    $funktionFunktionar = $fsFunk->addElement('text', 'addFunk', 
            array('style' => 'width: 300px;'), 
            array('label' => 'Ny funktion <br /><i>
                <small>(sekreterare, lärare, ...)</small></i>') );
    $funktionFunktionar->addRule('maxlength', 
        'Funktionsnamnet får max vara 50 tecken.', 50);
}
...
if ($_SESSION['authorityUser'] == 'adm') { 
    ...
    if ($arrayMalsman[0]) {
        $fsMalsman->addElement('checkbox', 'delMalsman', 
            array('value' => $arrayMalsman[0]))
...
if ($_SESSION['authorityUser'] == 'adm') { 
    ...
    if ($arrayElev[0]) {
        $fsElev->addElement('checkbox', 'delElev', 
            array('value' => $arrayElev[0]))
    ...
    $fsElev->addElement('static', 'comment')
        ->setContent('<small><i>Fyll i svenskt personnummer om eleven har det, 
            annars födelsedatum.</i></small>');
    $personnummerElev = $fsElev->addElement('text', 'personnummer', 
        array('style' => 'width: 300px;'), 
        array('label' => 'Personnummer') );
    $personnummerElev->addRule('regex', 'Personnumret måste ha formen 
        ååååmmdd-nnnn. Födelsedatum formen ååååmmdd.', 
        '/^(19|20)\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])(-\d{4})?$/');

    $gruppElev = $fsElev->addElement('text', 'grupp', 
        array('style' => 'width: 300px;'), 
        array('label' => 'Vilken grupp är eleven i') );
    $gruppElev->addRule('maxlength', 
        'Gruppnamnet är för långt för databasen.', 10);

    $nationalitetElev = $fsElev->addElement('select', 'nat', null, 
        array('options' => $nationality, 'label' => 'Nationalitet') );

    $arskursElev = $fsElev->addElement('text', 'grade', 
        array('style' => 'width: 300px;'), 
        array('label' => 'Årskurs i ordinarie skola') );
    $arskursElev->addRule('maxlength', 
        'Årskursen kan bara bestå av 2 tecken.', 2);

    $skolaElev = $fsElev->addElement('text', 'skola', 
        array('style' => 'width: 300px;'), 
        array('label' => 'Ordinarie skola') );
    $skolaElev->addRule('maxlength', 
        'Skolnamnet kan bara bestå av 50 tecken.', 50);

    if (strcmp("fnk", $_SESSION['authorityUser']) > 0 ) {
        $betaltElev = $fsElev->addElement('text', 'pay', 
            array('style' => 'width: 300px;'), 
            array('label' => 'Senast betalt') );
        $betaltElev->addRule('maxlength', 
            'Senast betalt kan bara bestå av 10 tecken.', 10);
    }
    ...
    if (isset($aMalsmanElev)) { 
        ...
        foreach($aMalsmanElev as $malsmanElev) {
            $fsElev->addElement('checkbox', 'delMalsmanElev', 
                array('value' => $malsmanElev['id']))
    ...
    $query = "
        SELECT idPerson, fornamnPerson, efternamnPerson 
            FROM {$tablePerson}	INNER JOIN {$tableMalsman}
            ON {$tablePerson}.idPerson = {$tableMalsman}.malsman_idPerson 
            ORDER BY efternamnPerson;";
    $result = $dbAccess->SingleQuery($query);
    while($row = $result->fetch_object()) {
        $malsmanList[$row->idPerson] = $row->fornamnPerson." "
            .$row->efternamnPerson;
    }
    $result->close();
    $fsElev->addElement('select', 'addMalsmanElev', 
        array('multiple' => 'multiple', 'size' => 3),
        array('options' => $malsmanList, 'label' => 'Välj en eller flera nya 
            målsmän<br /><small>Håll ner ctrl för flera</small>'));
}
...
if ($_SESSION['authorityUser'] == 'adm') { 
    if (isset($aMalsmanElev)) { 
        $fsBostad->addElement('checkbox', 'sammaBostadSomMalsman', 
            array('value' => $aMalsmanElev[0]['id']))
...
if ($_SESSION['authorityUser'] == 'adm') { 
    ...
    $bostadLista = array("" => "");
    while($row = $result->fetch_object()) {
        $bostadLista[$row->idPerson] = $row->fornamnPerson." "
            .$row->efternamnPerson;
    }
    $result->close();
    $fsBostad->addElement('select', 'sammaBostadSomAnnan', null, 
        array('options' => $bostadLista, 'label' => 'Samma bostad som') );
}

// Eller editera bostadsadress
$adressBostad = $fsBostad->addElement('text', 'adress', 
    array('style' => 'width: 300px;'), 
    array('label' => 'Adress') );
$adressBostad->addRule('maxlength', 
    'Adressen är för lång för databasen. Max 100 tecken.', 100);

$stadsdelBostad = $fsBostad->addElement('text', 'stadsdel', 
    array('style' => 'width: 300px;'), 
    array('label' => 'Stadsdel') );
$stadsdelBostad->addRule('maxlength', 
    'Stadsdelen är för lång för databasen.', 20);

$postnummerBostad = $fsBostad->addElement('text', 'postnummer', 
    array('style' => 'width: 300px;'), 
    array('label' => 'Postnummer') );
$postnummerBostad->addRule('maxlength', 
    'Postnumret är för lång för databasen.', 10);

$statBostad = $fsBostad->addElement('text', 'stat', 
    array('style' => 'width: 300px;'), 
    array('label' => 'Stat') );
$statBostad->addRule('maxlength', 'Statsnamnet är för långt för databasen.', 20);

$telefonBostad = $fsBostad->addElement('text', 'telefon', 
    array('style' => 'width: 300px;'), 
    array('label' => 'Telefonnummer bostad') );
$telefonBostad->addRule('maxlength', 
    'Telefonnumret är för långt för databasen.', 20);
...
$buttons->addElement('static', 'resetButton')
    ->setContent('<a title="Återställ" href="?p=edit_usr&amp;id='.$idPerson.'" >
...
if ($form->validate()) {
    ...
    if ($_SESSION['authorityUser'] == 'adm') { 
        if (isset($formValues['delMalsman'])) {
            ...
            if (isset($formValues['addMalsman'])) {
                ...
                // Om personen inte inte är målsman och heller inte ska bli 
                // målsman så finns inte personen i tabellen målsman och
                // därför blir queryn bara ignorerad.
                $query = "
                    UPDATE {$tableMalsman} 
                    SET nationalitetMalsman = '{$nationalitetMalsman}' 
                    WHERE malsman_idPerson = '{$idPerson}';";
            }
            $dbAccess->SingleQuery($query);
        }
    ...
    if ($_SESSION['authorityUser'] == 'adm') { 
        if (isset($formValues['delElev'])) {
            ...
            $dbAccess->SingleQuery($query);
            $query = "
                DELETE FROM {$tableElev} 
                WHERE elev_idPerson = '{$idPerson}';";
            $dbAccess->SingleQuery($query);
        } else {
            $personnummerElev = $dbAccess->WashParameter(
                strip_tags($formValues['personnummer']));
            $gruppElev        = $dbAccess->WashParameter(
                strip_tags($formValues['grupp']));
            $nationalitetElev = $dbAccess->WashParameter(
                strip_tags($formValues['nat']));
            $arskursElev      = $dbAccess->WashParameter(
                strip_tags($formValues['grade']));
            $skolaElev        = $dbAccess->WashParameter(
                strip_tags($formValues['skola']));
            $betaltElev       = $dbAccess->WashParameter(
                strip_tags($formValues['pay']));
            if (isset($formValues['addElev'])) {
                ...
                // Om personen inte inte är elev och heller inte ska bli elev 
                // så finns inte personen i tabellen elev och
                // därför blir queryn bara ignorerad.
    ...
    if (isset($formValues['delMalsmanElev'])) {
        ...
        $query = "
            DELETE FROM {$tableRelation} 
            WHERE relation_idMalsman = '{$idMalsman}' 
                AND relation_idElev='{$idPerson}';";
        $dbAccess->SingleQuery($query);
    }
...
QUERY;
    ...
    $sammaBostadSom = "";
    if (isset($formValues['sammaBostadSomAnnan'])) 
        $sammaBostadSom = $formValues['sammaBostadSomAnnan'];
    if (isset($formValues['sammaBostadSomMalsman'])) {
        if ($formValues['sammaBostadSomMalsman'] == 'same') 
            $sammaBostadSom = $newMalsman;
        else 
            $sammaBostadSom = $formValues['sammaBostadSomMalsman'];
    }
    ...
    if ($sammaBostadSom) {
        ...
        $query = "
            SELECT person_idBostad FROM {$tablePerson} 
            WHERE idPerson = '{$sammaBostadSom}';";
        $result = $dbAccess->SingleQuery($query);
        ...
        $telefonBostad    = $dbAccess->WashParameter(
            strip_tags($formValues['telefon']));
        $adressBostad     = $dbAccess->WashParameter(
            strip_tags($formValues['adress']));
        $stadsdelBostad   = $dbAccess->WashParameter(
            strip_tags($formValues['stadsdel']));
        $postnummerBostad = $dbAccess->WashParameter(
            strip_tags($formValues['postnummer']));
        $statBostad       = $dbAccess->WashParameter(
            strip_tags($formValues['stat']));
        ...
        if ($idBostad) { 
            ...
            if (!$adressBostad) 
                ...
                $adressBostad = "Temporär adress för ".$fornamnPerson." "
                    .$efternamnPerson;
            $query = "
